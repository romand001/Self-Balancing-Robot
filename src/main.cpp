...
double angle_kI;
double angle_kD;
const uint8_t angleBufferSize = 500;
double angleSum = 0;
...
double velocity_kI;
double velocity_kD;
const uint8_t velocityBufferSize = 500;
...
void IRAM_ATTR onTimer0() {
    ...
    digitalWrite(STEP1, pulseState0);
}
...
void IRAM_ATTR onTimer1() {
    ...
    digitalWrite(STEP2, pulseState1);
}
...
void loop() {
    ...
    double angle = angleRad * 180.0/PI;
    double angleError = angleRad - targetAngle;
    angleBuffer.push_back(angleError);
    uint8_t currentAngleBufferSize = angleBuffer.size();
    ...
    uint8_t currentVelocityBufferSize = velocityBuffer.size();
        ...
        if (mode == TESTING) {
            ...
            switch (rcvMsg[0]) {
                case 'f':
                    switch (rcvMsg[1]) {
                        case '1':
                            frequency0 = rcvMsg.substring(2,rcvMsg.length()-1).toInt()*10;
                            setFrequency(0, frequency0);
                        break;
                        case '2':
                            frequency1 = rcvMsg.substring(2,rcvMsg.length()-1).toInt()*10;
                            setFrequency(1, frequency1);
                        break;
                    }
                break;
                ...
                case 'm':
                    switch (rcvMsg[1]) {
                        ...
                        case '0':
                            digitalWrite(M0, rcvMsg.substring(2,3).toInt());
                        break;
                        case '1':
                            digitalWrite(M1, rcvMsg.substring(2,3).toInt());
                        break;
                        case '2':
                            digitalWrite(M2, rcvMsg.substring(2,3).toInt());
                        break;
                    }
                break;
    ...
    if (dataReady && currentTXTime - lastTXTime >= 10) {
